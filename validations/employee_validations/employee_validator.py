...
def validate_employee(employee: AbstractEmployee):
    if not validate_job(employee.job):
        logger.log(logging.ERROR, "Employee Job not valid")
        return False

    if employee.length_of_work > 55:
        logger.log(logging.ERROR, "Employee Length of Work is greater than 55 years")
        return False

    if employee.contract_payment <= 0:
        logger.log(logging.ERROR, "Employee Contract Payment can't be less than 0")
        return False
